...
void mglVertexArrayAttribFormat(GLMContext ctx, GLuint vaobj, GLuint attribindex, GLint size, GLenum type, GLboolean normalized, GLuint relativeoffset)
{
    ...
    assert(attribindex < ctx->state.max_vertex_attribs);

    ptr->attrib[attribindex].size = size;
    ptr->attrib[attribindex].type = type;
    ptr->attrib[attribindex].normalized = normalized;
    ptr->attrib[attribindex].relativeoffset = relativeoffset;
