        ...
        if (isBuffer(ctx, buffer))
        {
            ...
            if (VAO())
            {
                ...
                switch(target)
                {
                    ...
                    case GL_ATOMIC_COUNTER_BUFFER:
                    {
                        for(int attr=0; attr<MAX_ATTRIBS; attr++)
                        {
                            if (VAO_STATE(buffer_base.targets[attr].buffer))
                            {
                                if (VAO_STATE(buffer_base.targets[attr].buffer->target) == target)
                                {
                                    if (VAO_STATE(buffer_base.targets[attr].buffer->name) == buffer)
                                    {
                                        VAO_STATE(buffer_base.enabled_mask) &= ~(0x1 << attr);
                                        bzero(&VAO_STATE(buffer_base.targets[attr]), sizeof(BufferBaseTarget));
                                    }
                                }
                            }
                        }
...
void mglBindBuffer(GLMContext ctx, GLenum target, GLuint buffer)
{
    ...
    ERROR_CHECK_RETURN(buffer < ctx->state.buffer_table.current_name, GL_INVALID_VALUE);
...
void mglBindBufferBase(GLMContext ctx, GLenum target, GLuint index, GLuint buffer)
{
    ...
    if (isBuffer(ctx, buffer) == GL_FALSE)
    {
        assert(0);
    }
    ...
    if (buffer)
    {
        ...
        VAO_STATE(buffer_base.enabled_mask) |= 0x1 << index;
        VAO_STATE(buffer_base.targets[index].buffer) = ptr;
        VAO_STATE(buffer_base.targets[index].offset) = 0;
        VAO_STATE(buffer_base.targets[index].size) = ptr->size;
    }
    else
    {
        VAO_STATE(buffer_base.enabled_mask) &= ~(0x1 << index);
        bzero(&VAO_STATE(buffer_base.targets[index]), sizeof(BufferBaseTarget));
    }
}
...
void mglBindBufferRange(GLMContext ctx, GLenum target, GLuint index, GLuint buffer, GLintptr offset, GLsizeiptr size)
{
    ...
    if (buffer)
    {
        ...
        VAO_STATE(buffer_base.enabled_mask) |= 0x1 << index;
        VAO_STATE(buffer_base.targets[index].buffer) = ptr;
        VAO_STATE(buffer_base.targets[index].offset) = offset;
        VAO_STATE(buffer_base.targets[index].size) = size;
    }
    else
    {
        VAO_STATE(buffer_base.enabled_mask) &= ~(0x1 << index);
