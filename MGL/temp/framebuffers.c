...
//  mgl
//
//  Created by Michael Larson on 11/18/21.
//
...
extern void createTextureLevel(GLMContext ctx, Texture *tex, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLsizei depth, GLint border, GLenum format, GLenum type, const void *pixels);
...
GLenum  mglCheckFramebufferStatus(GLMContext ctx, GLenum target)
{
    ...
    width = tex->levels[level].width;
    height = tex->levels[level].height;
...
void mglRenderbufferStorage(GLMContext ctx, GLenum target, GLenum internalformat, GLsizei width, GLsizei height)
{
    ...
    createTextureLevel(ctx, tex, 0, internalformat, width, height, 0, 0, 0, 0, 0);
...
void mglGetRenderbufferParameteriv(GLMContext ctx, GLenum target, GLenum pname, GLint *params)
{
    ...
    switch(pname)
    {
        case GL_RENDERBUFFER_WIDTH:
            *params = RENDBUF_STATE(tex->levels[0].width); break;

        case GL_RENDERBUFFER_HEIGHT:
            *params = RENDBUF_STATE(tex->levels[0].height); break;

        case GL_RENDERBUFFER_INTERNAL_FORMAT:
            *params = RENDBUF_STATE(tex->levels[0].internalformat); break;
        ...
        case GL_RENDERBUFFER_DEPTH_SIZE:
            *params = RENDBUF_STATE(tex->levels[0].width); assert(0); break;
...
void getFramebufferAttachmentParameteriv(GLMContext ctx, GLuint framebuffer, GLenum target, GLenum attachment, GLenum pname, GLint *params)
{
    ...
    if (fbo)
    {
        ...
        {
            tex_level = &fbo_attachment_ptr->buf.rbo->tex->levels[level];
        }
        else
        {
            tex_level = &fbo_attachment_ptr->buf.tex->levels[level];
        }

        switch(pname)
        {
            case GL_FRAMEBUFFER_ATTACHMENT_RED_SIZE:
                *params = bicountForFormatType(tex_level->format, tex_level->type, GL_RED);
                return;

            case GL_FRAMEBUFFER_ATTACHMENT_GREEN_SIZE:
                *params = bicountForFormatType(tex_level->format, tex_level->type, GL_GREEN);
                return;

            case GL_FRAMEBUFFER_ATTACHMENT_BLUE_SIZE:
                *params = bicountForFormatType(tex_level->format, tex_level->type, GL_BLUE);
                return;

            case GL_FRAMEBUFFER_ATTACHMENT_ALPHA_SIZE:
                *params = bicountForFormatType(tex_level->format, tex_level->type, GL_ALPHA);
                return;

            case GL_FRAMEBUFFER_ATTACHMENT_DEPTH_SIZE:
            case GL_FRAMEBUFFER_ATTACHMENT_STENCIL_SIZE:
                *params = bicountForFormatType(tex_level->format, tex_level->type, GL_NONE);
                return;
